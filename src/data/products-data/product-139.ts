import { type Product } from '@/lib/types';

const product: Product = {
  id: 'product-139',
  name: 'طقم غلاية على قاعدة مع ابريق شاي هدية',
  slug: 'kettle-and-teapot-set',
  story: `جلسة الشاي لها أصولها. ☕ هذا الطقم الأنيق هو كل ما تحتاجه لتحضير الشاي على أصوله. غلاية سريعة، وإبريق يحافظ على دفء الشاي. ليس مجرد طقم، بل هو دعوة لجلسة "روقان" مع الأهل والأصدقاء.`,
  description: `طقم شاي متكامل من DENX يتكون من غلاية كهربائية 2 لتر وإبريق شاي زجاجي 0.8 لتر على قاعدة تسخين مزدوجة. مثالي للاستخدام العائلي والضيوف.###FEATURES###- غلاية كهربائية سعة 2 لتر.
- إبريق شاي زجاجي 0.8 لتر مع مصفاة.
- قاعدة تسخين مزدوجة.
- زر تشغيل وإيقاف بلمسة واحدة.
- وظيفة الحفاظ على الشاي دافئًا.###DETAILS###- الماركة: DENX
- الموديل: DX1788
- القوة: 1800 واط
- السعة: 2 لتر (غلاية) + 0.8 لتر (إبريق)`,
  price: 150.00,
  images: [
    'https://i.ibb.co/bMVsgZZW/image-1.jpg',
    'https://i.ibb.co/pBTxCtsZ/image-2.jpg',
    'https://i.ibb.co/BVYk4wKK/image-3.jpg',
    'https://i.ibb.co/pBDGfTn0/image-4.jpg',
    'https://i.ibb.co/TDk0kNH6/image-5.jpg'
  ],
  category: 'home',
  warranty: 'عامين',
};

export default product;
