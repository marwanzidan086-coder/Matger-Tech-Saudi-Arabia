
import { type Product } from '@/lib/types';
import { PlaceHolderImages } from '@/lib/placeholder-images';
import { categories } from '@/lib/categories';

const getImages = (productId: string): string[] => {
  const imageSet = PlaceHolderImages.find(img => img.id === productId);
  if (imageSet && imageSet.images.length > 0) {
    return imageSet.images;
  }
  // Fallback in case images are not found for a specific product
  return Array.from({length: 1}, (_, i) => `https://picsum.photos/seed/${productId}-${i + 1}/600/600`);
};

const allProducts: Product[] = [
  {
    id: 'product-1',
    name: 'راعى الكشتات فانوس متعدد الاستخدامات مع بور بنك من DLC',
    slug: 'dlc-multi-use-lantern-power-bank',
    description: `استعد لأي مغامرة مع فانوس DLC، رفيقك الأمثل في كل رحلة! سواء كنت في البر، في رحلة تخييم، أو حتى في حالة طوارئ بالمنزل، هذا الفانوس يجمع لك بين الإضاءة القوية والشحن المتنقل. بفضل تصميمه الذكي والمواد المتينة، هو أكثر من مجرد فانوس، بل هو أداة لا غنى عنها في كل بيت وسيارة.###FEATURES###إضاءة LED قوية تنور لك المكان بوضوح.
شحن USB سريع وما ينقطع بسهولة.
تصميم مغناطيسي يثبت على أي سطح معدني.
قابل للتعديل بزوايا تناسب احتياجك.
مقاوم للمطر والغبار والظروف القاسية.
بطارية قوية تدوم معك لفترة طويلة.
خفيف الوزن وسهل تحمله بأي مكان.
مناسب للبيت والرحلات والطوارئ.
سهل الفك والتركيب والاستخدام اليومي.
مصنوع من خامات قوية تعيش معك أكثر.###DETAILS###الماركة: DLC
الموديل: DLC-32898
الضمان: عامين
القدرة: 6 – 10 واط`,
    price: 119.99,
    images: getImages('product-1'),
    category: 'electronics',
    warranty: 'عامين',
    story: "في ليلة ظلماء، وأثناء رحلة تخييم في قلب الصحراء، انقطعت الكهرباء فجأة. لكن بفضل فانوس DLC، لم تكن هناك مشكلة. بقوته وضوئه الساطع، تحولت العتمة إلى نور، واستمرت السهرة وكأن شيئًا لم يكن، بل وشحن هواتف الجميع ليظلوا على تواصل.",
    faq: [
      {
        question: "هل يمكنني استخدامه أثناء المطر؟",
        answer: "نعم، الفانوس مقاوم للمطر والغبار، مما يجعله مثاليًا للاستخدام في الظروف الجوية المختلفة."
      },
      {
        question: "كم تدوم بطاريته تقريبًا؟",
        answer: "يحتوي الفانوس على بطارية قوية تدوم لفترة طويلة، مما يجعله رفيقًا موثوقًا في الرحلات الطويلة والطوارئ."
      }
    ]
  },
  {
    id: 'product-2',
    name: 'كشاف دينكس الأكبر حجما',
    slug: 'denx-large-solar-spotlight',
    description: `ودّع فواتير الكهرباء المرتفعة وأنر مساحاتك الخارجية بقوة الشمس! كشاف دينكس هو الحل الأمثل لإضاءة الحدائق، الممرات، والمزارع بكفاءة واعتمادية. يجمع بين القوة والمتانة والتكنولوجيا الذكية ليوفر لك إضاءة مستمرة وآمنة طوال الليل.###FEATURES###إضاءة قوية بفضل لمبات LED 5730 الموفرة.
يشتغل بالطاقة الشمسية بدون كهرباء أو أسلاك.
بطارية 25000mAh تدوم حتى 16 ساعة.
يدعم التحكم بالريموت عن بعد بكل سهولة.
فيه حساس رادار يشتغل أوتوماتيكي مع الحركة.
مقاوم للمطر والغبار بمعيار حماية IP67.
يتحمل أصعب الأجواء مثل المطر والعواصف الرملية.
إضاءة ذكية تشتغل تلقائي مع الغروب وتطفي مع الشروق.###DETAILS###الماركة: دينيكس
الضمان: عامين
نوع اللمبات: LED 5730
سعة البطارية: 25000mAh ليثيوم
مدة الإضاءة: حتى 16 ساعة بالشحنة الكاملة
معيار الحماية: IP67`,
    price: 249.99,
    images: getImages('product-2'),
    category: 'electronics',
    warranty: 'عامين'
  },
  {
    id: 'product-3',
    name: 'شاشة الفارس 32 بوصة',
    slug: 'alfaris-32-inch-screen',
    description: `حوّل غرفة جلوسك إلى سينما صغيرة مع شاشة الفارس! استمتع بألوان زاهية وتفاصيل واضحة تجعل كل مشهد ينبض بالحياة. تصميمها الأنيق وحجمها المثالي يجعلانها الخيار الأمثل لكل من يبحث عن جودة ومتعة بسعر في متناول اليد.###FEATURES###ألوان واضحة وحية تعطيك تجربة مشاهدة ممتعة.
يدعم مدخل HDMI و USB لتوصيل مختلف الأجهزة.
نظام AVL يعطي صوت متوازن بدون تشويش.
إضاءة LED أوضح وتوفير أكثر للطاقة.
مقاس 32 بوصة مناسب للغرف المتوسطة.
دقة HD Ready تعطي تفاصيل أوضح بالصورة.
إطارات نحيفة تمنح الشاشة شكلاً أنيقاً.###DETAILS###الماركة: الفارس
الضمان: 5 سنين
المقاس: 32 بوصة
الدقة: 720p HD Ready
المنافذ: HDMI + USB
القدرة: 48 واط`,
    price: 499.99,
    images: getImages('product-3'),
    category: 'electronics',
    warranty: '5 سنين'
  },
  {
    id: 'product-4',
    name: 'جهاز ألعاب حديث R36S',
    slug: 'r36s-retro-gaming-console',
    description: `استرجع أجمل ذكريات الطفولة مع جهاز الألعاب المحمول R36S! آلاف الألعاب الكلاسيكية بين يديك في جهاز واحد أنيق وقوي. سواء كنت من محبي ألعاب الآركيد القديمة أو مغامرات المنصات، هذا الجهاز هو بوابتك لعالم من المرح لا ينتهي.###FEATURES###يحتوي على أكثر من 10,000 لعبة.
نظام Linux سريع وسهل التصفح.
شاشة 3.5 إنش بدقة 640×480.
معالج رباعي النواة قوي 1.5GHz.
بطارية 3500mAh تدوم لساعات.
ذاكرة 128 جيجا لتخزين الألعاب.###DETAILS###نظام التشغيل: Linux
المعالج: ARM Cortex-A35
الرام: 1GB DDR3L
السعة التخزينية: 128GB
الشاشة: 3.5 إنش (640×480)
البطارية: 3500mAh`,
    price: 299.99,
    images: getImages('product-4'),
    category: 'electronics'
  },
  {
    id: 'product-5',
    name: 'بور بنك وكشاف بالطاقة الشمسية من دينكس',
    slug: 'denx-solar-power-bank-spotlight',
    description: `لا تدع بطارية هاتفك تنفد مرة أخرى، حتى في قلب الطبيعة! بور بنك دينكس الشمسي هو الحل الشامل للطاقة أثناء التنقل. مع قدرته على الشحن من الشمس والشحن اللاسلكي، هو الرفيق المثالي لكل رحالة ومغامر.###FEATURES###يشحن بالطاقة الشمسية ويناسب الطلعات والسفر.
فيه شحن لاسلكي سريع ومريح.
يدعم شحن أكثر من جهاز بنفس الوقت.
مزود بألواح شمسية قابلة للطي.
إضاءته قوية تنفع للطوارئ والرحلات.
مناسب للجوالات، التابلت، والسماعات.###DETAILS###الموديل: DX421
الماركة: دينيكس
الضمان: عامين
سعة البطارية: 20000 mAh
مخرج لاسلكي: 5V/1A
قدرة الطاقة الشمسية: 5V/0.3A`,
    price: 159.99,
    images: getImages('product-5'),
    category: 'electronics',
    warranty: 'عامين'
  },
  {
    id: 'product-6',
    name: 'هارد محمول سعة 500 جيجابايت',
    slug: 'portable-hard-drive-500gb',
    description: `احتفظ بذكرياتك وملفاتك المهمة بأمان وأناقة! هذا الهارد المحمول ليس مجرد مساحة تخزين، بل هو قلعة متينة لبياناتك. بتصميمه المعدني الأنيق وسرعته الفائقة، هو الخيار الأمثل للمحترفين والطلاب وكل من يحتاج إلى أداء موثوق.###FEATURES###سعة 500GB لتخزين كل ملفاتك.
واجهة USB 3.0 عالية السرعة لنقل سريع.
تصميم معدني أنيق ومتين.
مقاوم للصدمات والخدوش.
تشغيل مباشر بدون برامج (Plug & Play).
متوافق مع ويندوز وماك.###DETAILS###الحجم: 2.5 إنش
السرعة: 5400 RPM
الواجهة: USB 3.0
المادة: معدن
الضمان: 3 سنوات`,
    price: 189.99,
    images: getImages('product-6'),
    category: 'electronics',
    warranty: '3 سنوات'
  },
  {
    id: 'product-7',
    name: 'ست عواميد إنارة بالطاقة الشمسية',
    slug: 'six-solar-light-poles',
    description: `أضف لمسة من السحر والأمان لحديقتك أو ممراتك مع أعمدة الإنارة الشمسية. لا تحتاج إلى كهرباء أو تمديدات، فقط دعها تستمد قوتها من الشمس لتنير ليلك بضوء دافئ وجذاب. تصميمها الأنيق ومقاومتها للطقس يجعلانها إضافة مثالية لأي مساحة خارجية.###FEATURES###تشتغل بالطاقة الشمسية، لا تحتاج كهرباء.
تشحن نهارًا وتضيء تلقائيًا في الليل.
مقاومة للمطر والغبار (تصنيف IP65).
تحتوي على 60 لمبة LED قوية.
عمر افتراضي طويل يصل إلى 50 ألف ساعة.
إضاءة متعددة الخيارات: أبيض دافئ أو ساطع.###DETAILS###الماركة: جينيريك
الضمان: 14 يوم
البطارية: 3.7V 2000mAh
وقت الشحن: 5-6 ساعات
وقت التشغيل: 6 إلى 12 ساعة
المواد: ABS + PC`,
    price: 199.99,
    images: getImages('product-7'),
    category: 'home',
    warranty: '14 يوم'
  },
  {
    id: 'product-8',
    name: 'باور بنك 10000 ملى امبير من دينكس',
    slug: 'denx-10000mah-power-bank',
    description: `القوة والسرعة في جيبك! باور بنك دينكس بقوة 30 واط هو شريان الحياة لأجهزتك. بحجمه الصغير وقدرته على الشحن السريع، لن تقلق أبدًا بشأن نفاد البطارية في منتصف يوم حافل.###FEATURES###قوة شحن 30 واط لشحن فائق السرعة.
سعة 10000 mAh تكفيك لشحن جوالك أكثر من مرة.
حجم صغير ومناسب للجيب أو الشنطة.
منافذ متعددة لشحن أكثر من جهاز.
حماية من الحرارة الزائدة والتماس الكهربائي.###DETAILS###الماركة: دينيكس
الضمان: عامين
الموديل: DX432
القدرة: 30W
السعة: 10000 mAh
الخامة: ABS + PC`,
    price: 89.99,
    images: getImages('product-8'),
    category: 'electronics',
    warranty: 'عامين'
  },
  {
    id: 'product-9',
    name: 'كاميرا الفلوج الذكية ضد الماء',
    slug: 'waterproof-smart-vlogging-camera',
    description: `وثّق مغامراتك فوق وتحت الماء! كاميرا الفلوج الذكية هي رفيقك المثالي لتصوير كل لحظة مثيرة. بغلافها المقاوم للماء وجودتها العالية، يمكنك التقاط صور ومقاطع فيديو مذهلة في أي مكان، من أعماق البحر إلى قمم الجبال.###FEATURES###تأتي مع غلاف مقاوم للماء للتصوير تحت الماء.
ممتازة لتصوير الفلوقات والسفر والمغامرات.
عدسة واسعة 140 درجة لالتقاط مشهد كامل.
تصوير بدقّة 4K لجودة احترافية.
تصميم صغير وخفيف، يسهل حملها.
تدعم كروت ذاكرة حتى 32 جيجا.###DETAILS###الماركة: جينيريك
الضمان: 14 يوم
دقة الفيديو: 1080p / 4K
مقاومة للماء: حتى 30 متر
زاوية العدسة: 140 درجة
حجم الشاشة: 2 إنش`,
    price: 129.99,
    images: getImages('product-9'),
    category: 'electronics',
    warranty: '14 يوم'
  },
  {
    id: 'product-10',
    name: 'سماعة بحامل مانع للسقوط',
    slug: 'anti-fall-headset-with-holder',
    description: `استمتع بموسيقاك دون انقطاع أو قلق من سقوط السماعات. بفضل تصميمها المغناطيسي المبتكر وبطاريتها التي تدوم طويلًا، تمنحك هذه السماعة حرية الحركة والراحة التامة، سواء كنت في النادي الرياضي أو في طريقك إلى العمل.###FEATURES###بلوتوث 5.0 لاتصال سريع ومستقر.
بطارية قوية تدوم تقريبًا 30 ساعة.
تصميم مغناطيسي يحول السماعات إلى عقد أنيق.
مقاومة للماء والعرق، مثالية للرياضة.
سطح ناعم من السيليكون لراحة فائقة.
تحكم ذكي بالموسيقى والمكالمات.###DETAILS###الماركة: BASTEC
الضمان: 14 يوم
إصدار البلوتوث: V5.0
مسافة التشغيل: 10 أمتار
سعة البطارية: 250mAh
مدة الاستخدام: حوالي 30 ساعة`,
    price: 79.99,
    images: getImages('product-10'),
    category: 'electronics',
    warranty: '14 يوم'
  },
  {
    id: 'product-11',
    name: 'محول الجوال الى جهاز كمبيوتر من دينكس',
    slug: 'denx-mobile-to-pc-adapter',
    description: `حوّل جوالك إلى محطة عمل متكاملة! محول دينكس هو الحل السحري لتوصيل كل ملحقاتك بجهاز واحد. اعرض على شاشة كبيرة، انقل ملفاتك بسرعة، واقرأ كروت الذاكرة، كل ذلك من خلال منفذ Type-C في جوالك أو لابتوبك.###FEATURES###يعرض لك فيديو بجودة 4K عن طريق HDMI.
سرعة نقل عالية عبر منافذ USB (توصل 5Gbps).
فيه قارئ كروت TF/SD للصور والملفات.
يدعم أحدث تقنية Type-C.
متوافق مع اللابتوبات، الجوالات، والتابلت.
يغنيك عن كذا قطعة، يجمع لك كل المنافذ بقطعة وحدة.###DETAILS###الماركة: دينكس
الضمان: عامين
إخراج الفيديو: 4K عبر HDMI
المنافذ: USB, Type-C, SD/TF
سرعة النقل: تصل إلى 5Gbps`,
    price: 149.99,
    images: getImages('product-11'),
    category: 'electronics',
    warranty: 'عامين'
  },
  {
    id: 'product-12',
    name: 'تابلت 07 بكيبورد وماوس بلوتوث',
    slug: 'tablet-07-with-keyboard-mouse',
    description: `أطلق العنان لإبداعك وإنتاجيتك مع تابلت 07! جهاز لوحي متكامل يأتي مع كيبورد وماوس وقلم، ليتحول إلى لابتوب صغير وقتما تشاء. بشاشته الكبيرة وأدائه القوي، هو خيارك الأمثل للدراسة، العمل، والترفيه.###FEATURES###شاشة كبيرة 7 إنش بدقة FHD.
ذاكرة RAM ضخمة 16GB لتشغيل سلس.
سعة تخزين 1TB لحفظ كل ملفاتك.
كاميرا خلفية 30MP وأمامية 13MP.
نظام Android 13 حديث.
بطارية 8000mAh تدوم طويلاً.
يدعم شريحة اتصال 5G.###DETAILS###حجم الشاشة: 7 بوصة
الذاكرة الداخلية: 1 تيرابايت
الذاكرة العشوائية: 16 جيجا رام
الكاميرا الخلفية: 30 ميجا بكسل
الكاميرا الأمامية: 13 ميجا بكسل
البطارية: 8000 مللي أمبير
الضمان: 6 أشهر`,
    price: 699.99,
    images: getImages('product-12'),
    category: 'electronics',
    warranty: '6 أشهر'
  },
  {
    id: 'product-13',
    name: 'حافظة مشروبات Moog max الذكية مع سبيكر',
    slug: 'moog-max-smart-bottle-speaker',
    description: `استمتع بمشروبك البارد أو الساخن مع إيقاع موسيقاك المفضلة! حافظة Moog Max ليست مجرد مطارة، بل هي رفيقك اليومي الذي يجمع بين الحفاظ على درجة حرارة مشروبك ومتعة الاستماع للموسيقى عبر سبيكر مدمج.###FEATURES###تحافظ على الحرارة والبرودة لمدة تصل إلى 12 ساعة.
مكبر صوت بلوتوث مدمج سهل الفك والتركيب.
مصنوعة من الستانلس ستيل عالي الجودة.
تصميم أنيق وعملي بسعة 500 مل.
قاعدة مقاومة للانزلاق وغطاء محكم.###DETAILS###السعة: 500 مل
الخامة: فولاذ مقاوم للصدأ
الأبعاد: 230 * 65 مللي متر
مكبر الصوت: بلوتوث 0.5`,
    price: 99.99,
    images: getImages('product-13'),
    category: 'home'
  },
  {
    id: 'product-14',
    name: 'كافر جوال وباوربنك لحماية أيفون 15 pro max',
    slug: 'iphone-15-pro-max-power-bank-case',
    description: `لا تشيل هم البطارية أو حماية جوالك بعد اليوم! هذا الكفر المبتكر يجمع بين الحماية القوية والباور بانك المدمج، ليمنح جهازك الآيفون 15 Pro Max عمرًا أطول ومظهرًا أنيقًا.###FEATURES###بطارية مدمجة بسعة 6800mAh.
تصميم نحيف وأنيق وخفيف الوزن.
يشحن الجهاز لاسلكيًا بلمسة واحدة.
يحمي الجوال من الصدمات والخدوش.
شاشة LED لعرض مستوى الشحن.###DETAILS###السعة: 6800mAh
متوافق مع: iPhone 15 Pro Max (6.7 انش)
الخامة: ABS + TPU
نوع البطارية: ليثيوم بوليمر
الإدخال: 5V/1.5A
الإخراج: 5V/1.0A`,
    price: 179.99,
    images: getImages('product-14'),
    category: 'electronics'
  },
  {
    id: 'product-15',
    name: 'بور بنك بكشاف بالطاقة الشمسية 5 أوضاع إضاءة',
    slug: 'solar-power-bank-with-spotlight',
    description: `رفيقك الأمثل في البر والطوارئ! هذا الجهاز ليس مجرد كشاف، بل هو محطة طاقة متنقلة تشحن من الشمس وتوفر لك إضاءة قوية بخمسة أوضاع مختلفة، بالإضافة إلى قدرته على شحن أجهزتك الصغيرة.###FEATURES###إضاءة قوية جداً بسطوع 6000 لومن.
تشتغل بالطاقة الشمسية بدون كهرباء.
مقاومة للماء ومناسبة للاستخدام الخارجي.
بطارية قابلة للشحن مدمجة.
شحن USB للطوارئ والأجهزة الصغيرة.###DETAILS###الضمان: 14 يوم
المادة: ABS
مقاوم للماء: نعم
زاوية الإضاءة: 140°
شدة السطوع: 6000 لومن
عمر المصباح: 50,000 ساعة`,
    price: 139.99,
    images: getImages('product-15'),
    category: 'electronics',
    warranty: '14 يوم'
  },
  {
    id: 'product-16',
    name: 'سماعة أيربودز من Noxon',
    slug: 'noxon-airpods',
    description: `انغمس في عالم من الصوت النقي مع سماعات Noxon! بتصميمها المريح وميزة عزل الضوضاء، ستستمتع بمكالماتك وموسيقاك بوضوح تام. تتصل بجوالك تلقائيًا بمجرد فتح العلبة.###FEATURES###تدعم عزل الضوضاء لسماع واضح.
تتصل تلقائيًا بالجوال.
تدعم التحكم باللمس بدون أزرار.
تتوافق مع كل الأنظمة (أندرويد/آيفون).
صوت نقي وواضح في المكالمات.###DETAILS###الماركة: نوكسون
الضمان: 14 يوم
وقت التشغيل: 2.5 ساعة مكالمات متواصلة
مدى الاتصال: 10 أمتار
إعادة الشحن من العلبة: 2–3 مرات`,
    price: 99.99,
    images: getImages('product-16'),
    category: 'electronics',
    warranty: '14 يوم'
  },
  {
    id: 'product-17',
    name: 'سماعة أيربودز i12 TWS',
    slug: 'i12-tws-airpods',
    description: `تجربة صوتية ممتازة بسعر لا يصدق! سماعات i12 TWS تقدم لك صوت ستيريو نقي وتصميمًا مريحًا يثبت في الأذن. اتصل بأجهزتك بسهولة عبر بلوتوث 5.0 واستمتع بحرية الحركة.###FEATURES###صوت ستيريو نقي وواضح.
تصميم مريح وخفيف الوزن.
تدعم البلوتوث 5.0 لاتصال سريع وثابت.
تتوافق مع الآيفون والأندرويد.
علبة الشحن توفر شحن إضافي.
تحكم باللمس للمكالمات والموسيقى.###DETAILS###الموديل: i12 TWS
الضمان: 14 يوم
إصدار البلوتوث: 5.0
مدة التشغيل: 2 إلى 3 ساعات
زمن الشحن: ساعة ونصف تقريبًا`,
    price: 69.99,
    images: getImages('product-17'),
    category: 'electronics',
    warranty: '14 يوم'
  },
  {
    id: 'product-18',
    name: 'عرض اليوم الوطني السعودي (حقيبة - ساعة - سماعة - باور بنك)',
    slug: 'saudi-national-day-bundle',
    description: `عرض متكامل يجمع لك كل ما تحتاجه في يومك! حقيبة كتف أنيقة مع منفذ USB، باور بانك سريع الشحن، ساعة ذكية لمتابعة نشاطك، وسماعة Type-C بصوت نقي. كل هذا في باقة واحدة رائعة.###FEATURES###حقيبة كتف أنيقة مع منفذ USB.
باور بانك 30 واط بسعة 10000mAh.
سماعة Type-C بصوت نقي وريموت تحكم.
ساعة ذكية تقيس نبضات القلب والنشاط.
جميع القطع خفيفة وسهلة الحمل.###DETAILS###الباور بنك: دينيكس 30W, 10000mAh
الساعة: Shivay M3 Smart Band
الحقيبة: yunfan, منفذ USB
السماعة: Type-C سلكية`,
    price: 249.99,
    images: getImages('product-18'),
    category: 'misc'
  },
  {
    id: 'product-19',
    name: 'كاميرا تصوير أحترافي+ عدسة',
    slug: 'professional-camera-with-lens',
    description: `أطلق العنان للمصور الذي بداخلك! هذه الكاميرا بدقة 16 ميجا بيكسل وزوم رقمي 16x تمنحك القدرة على التقاط صور ومقاطع فيديو رائعة. بتصميمها الخفيف وسهولة استخدامها، هي الخيار المثالي للمبتدئين والهواة.###FEATURES###زوم رقمي 16x لتقريب الصور.
دقة 16 ميجا بيكسل لصور واضحة.
تصوير فيديو بدقة 720p.
شاشة 2.4 إنش للمعاينة.
تثبيت إلكتروني لتقليل الاهتزاز.###DETAILS###الموديل: HD - 05
الضمان: 14 يوم
الدقة: 16 ميجا بيكسل
الزوم: 16x رقمي
الشاشة: 2.4 إنش
البطارية: 4 بطاريات AA (غير مرفقة)`,
    price: 299.99,
    images: getImages('product-19'),
    category: 'electronics',
    warranty: '14 يوم'
  },
  {
    id: 'product-20',
    name: 'ساعة يد ذكية بشاشة كاملة',
    slug: 'full-screen-smart-watch',
    description: `ابق على اتصال وتابع صحتك بأناقة! هذه الساعة الذكية بشاشتها الكاملة تتيح لك استقبال المكالمات والإشعارات، وقياس نبضات القلب وضغط الدم، كل ذلك من معصمك.###FEATURES###متابعة قياسات طبية (ضربات القلب، ضغط الدم).
ربط بالهاتف واستقبال المكالمات والإشعارات.
شاشة كاملة تعمل باللمس 1.54 بوصة.
بطارية 195 مللي أمبير.
شحن لاسلكي.###DETAILS###البلوتوث: 3.0 و 4.0
البطارية: 195 مللي أمبير
الشاشة: 1.54 HD IPS
الشحن: لاسلكي`,
    price: 159.99,
    images: getImages('product-20'),
    category: 'health'
  },
  {
    id: 'product-21',
    name: 'تابلت A20',
    slug: 'a20-tablet',
    description: `تجربة رقمية متكاملة في جهاز واحد! تابلت A20 بشاشته الكبيرة وأدائه السريع هو رفيقك المثالي للعمل، الدراسة، والترفيه. يأتي مع مجموعة هدايا قيمة تشمل سماعة بلوتوث وجراب.###FEATURES###شاشة 7 بوصة عالية الدقة.
مساحة 256 جيجابايت و 6 جيجا رام.
نظام تشغيل أندرويد 12.
بطارية 5000 مللي أمبير.
كاميرا أمامية 8.0MP وخلفية 13.0MP.###DETAILS###الاتصال: 5G/SIM/WIFI
الشاشة: 7 بوصة IPS LCD
الذاكرة: 256GB + 6GB RAM
نظام التشغيل: اندرويد 12
البطارية: 5000 مللي أمبير
الضمان: 6 أشهر`,
    price: 499.99,
    images: getImages('product-21'),
    category: 'electronics',
    warranty: '6 أشهر'
  },
  {
    id: 'product-22',
    name: 'ساعة سمارت ULTRA',
    slug: 'ultra-smart-watch',
    description: `أناقة وقوة على معصمك! ساعة ULTRA الذكية بتصميمها العصري وشاشتها عالية الدقة تمنحك كل ما تحتاجه لمتابعة لياقتك وصحتك، مع إمكانية استقبال المكالمات والتحكم في هاتفك بسهولة.###FEATURES###شاشة عالية الدقة وكاملة.
أزرار من الجهتين مثل ساعة آبل.
استقبال جميع الإشعارات والمكالمات.
قياس نبضات القلب، أكسجين الدم، وضغط الدم.
مقاومة للماء ومصممة بمواد متينة.###DETAILS###الشاشة: 1.96 inch HD screen
البلوتوث: BT 3.5 / 5.0
البطارية: 320 مللي أمبير
الشحن: لاسلكي`,
    price: 189.99,
    images: getImages('product-22'),
    category: 'health'
  },
  {
    id: 'product-23',
    name: 'كيبورد و ماوس كومبو للالعاب',
    slug: 'gaming-keyboard-mouse-combo',
    description: `أضف لمسة من الاحترافية والألوان إلى مكتبك! هذا الطقم المكون من كيبورد وماوس بإضاءة خلفية ملونة يمنحك تجربة لعب وكتابة مريحة وممتعة.###FEATURES###إضاءة LED خلفية بألوان قوس قزح.
تصميم مريح يقلل الإجهاد.
عجلة ماوس ثلاثية الأبعاد ومضادة للانزلاق.
يدعم أنظمة تشغيل ويندوز.
مصنوع من مواد عالية الجودة.###DETAILS###الكيبورد: 104 مفتاح، طول الكابل 1.5 متر
الماوس: إضاءة خلفية، طول الكابل 1.5 متر
اللون: عشوائي`,
    price: 129.99,
    images: getImages('product-23'),
    category: 'electronics'
  },
  {
    id: 'product-24',
    name: 'تابلت شبيه الايفون + كيبورد بلوتوث',
    slug: 'iphone-like-tablet-with-keyboard',
    description: `احصل على إنتاجية عالية وتصميم أنيق مع هذا التابلت الشبيه بالآيفون! يأتي مع كيبورد بلوتوث وذاكرة كبيرة ليساعدك على إنجاز مهامك بسرعة وسهولة، سواء في العمل أو أثناء التنقل.###FEATURES###ذاكرة رام 6 جيجا و مساحة 256 جيجابايت.
يدعم شريحة 5G.
يأتي مع كيبورد بلوتوث يدعم اللغة العربية.
شاشة 7 بوصة عالية الوضوح.
بطارية 5000 مللي أمبير تدوم طويلاً.###DETAILS###المساحة: 256GB
الرام: 6 جيجا
الشبكة: 5G
الشاشة: 7 بوصة
البطارية: 5000 مللي أمبير`,
    price: 549.99,
    images: getImages('product-24'),
    category: 'electronics'
  },
  {
    id: 'product-25',
    name: 'Note 15 مزود ب كيبورد',
    slug: 'note-15-with-keyboard',
    description: `أداء خارق وسرعة فائقة في جهاز واحد! تابلت Note 15 مع الكيبورد المتطور يقدم لك تجربة كتابة وعمل سلسة، بفضل معالجه القوي وذاكرته الضخمة، وشاشته الواضحة.###FEATURES###شاشة 8 بوصة عالية الدقة.
معالج رباعي النواة.
ذاكرة 16 جيجا رام و 512 جيجا بايت تخزين.
يدعم 5G، واي فاي، وبلوتوث.
بطارية 5000 مللي أمبير.###DETAILS###الشاشة: 8 بوصة
الذاكرة: 16GB RAM + 512GB ROM
الكاميرا: 13.0MP أمامية + 13.0MP خلفية
نظام التشغيل: أندرويد
البطارية: 5000 مللي أمبير`,
    price: 749.99,
    images: getImages('product-25'),
    category: 'electronics'
  },
  {
    id: 'product-26',
    name: 'ساعة سمارت Ultra T8',
    slug: 'smart-watch-ultra-t8',
    description: `كن دائمًا مميزًا مع ساعة سمارت Ultra T8! بتصميمها العصري وشاشتها الكبيرة عالية الدقة، يمكنك التحكم في جوالك، متابعة صحتك، واستقبال إشعاراتك بكل أناقة وسهولة.###FEATURES###شاشة كبيرة 1.96 بوصة عالية الدقة.
أزرار من الجهتين بتصميم عصري.
استقبال جميع الإشعارات والمكالمات.
قياس نبضات القلب، أكسجين الدم، وضغط الدم.
تدعم الشحن اللاسلكي.###DETAILS###الأبعاد: 49*44*12 سم
الشاشة: 1.96 inch HD screen
البلوتوث: BT 3.5 / 5.0
البطارية: 320 مللي أمبير
الشحن: لاسلكي`,
    price: 199.99,
    images: getImages('product-26'),
    category: 'health'
  },
  {
    id: 'product-27',
    name: 'جهاز ألعاب بدقة 4K مع دراعات تحكم',
    slug: '4k-gaming-console-with-controllers',
    description: `استرجع ذكريات الطفولة مع جهاز الألعاب هذا! آلاف الألعاب الكلاسيكية بانتظارك على شاشتك الكبيرة بدقة 4K. يأتي مع وحدتي تحكم لاسلكية لتجربة لعب مريحة وممتعة.###FEATURES###أكثر من 18,500 لعبة مدمجة.
يدعم 9 أنواع من محاكيات الألعاب.
يدعم دقة عرض 4K عبر مخرج HDMI.
وحدتين تحكم لاسلكية 2.4 جيجا.
إمكانية حفظ تقدم اللعبة.###DETAILS###عدد الألعاب: 18,500+
الاتصال: لاسلكي 2.4 جيجا
التخزين: بطاقة 64 جيجابايت
المحاكيات المدعومة: 9 أنواع`,
    price: 249.99,
    images: getImages('product-27'),
    category: 'electronics'
  },
  {
    id: 'product-28',
    name: 'جهاز السينما المتنقل الذكي',
    slug: 'smart-portable-cinema-projector',
    description: `حوّل أي مكان إلى سينما خاصة بك! هذا البروجكتر الذكي والمحمول يعرض صورة عالية الدقة تصل إلى 4K، مع تصميم دوار 180 درجة لعرض الصورة حتى على السقف.###FEATURES###تصميم دوار 180 درجة.
يدعم دقة 4K مع تصحيح تلقائي للانحراف.
نظام تشغيل أندرويد 11.0 مدمج.
مكبرات صوت ستيريو 5 واط.
يدعم الاتصال اللاسلكي بالهواتف.###DETAILS###نظام التشغيل: أندرويد 11.0
الدقة الأصلية: 1280 × 720 بكسل (يدعم 4K)
السطوع: 120 لومن ANSI
الاتصال: واي فاي، بلوتوث 4.1, HDMI`,
    price: 399.99,
    images: getImages('product-28'),
    category: 'electronics'
  },
  {
    id: 'product-29',
    name: 'جهاز السينما المتنقل الذكي - عرض خاص',
    slug: 'smart-portable-cinema-projector-special',
    description: `حوّل أي مكان إلى سينما خاصة بك! هذا البروجكتر الذكي والمحمول يعرض صورة عالية الدقة تصل إلى 4K، مع تصميم دوار 180 درجة لعرض الصورة حتى على السقف.###FEATURES###تصميم دوار 180 درجة.
يدعم دقة 4K مع تصحيح تلقائي للانحراف.
نظام تشغيل أندرويد 11.0 مدمج.
مكبرات صوت ستيريو 5 واط.
يدعم الاتصال اللاسلكي بالهواتف.###DETAILS###نظام التشغيل: أندرويد 11.0
الدقة الأصلية: 1280 × 720 بكسل (يدعم 4K)
السطوع: 120 لومن ANSI
الاتصال: واي فاي، بلوتوث 4.1, HDMI`,
    price: 399.99,
    images: getImages('product-29'),
    category: 'electronics'
  },
  {
    id: 'product-30',
    name: 'كافر جوال وباوربنك لحماية أيفون 13 pro max',
    slug: 'iphone-13-pro-max-power-bank-case',
    description: `لا تشيل هم البطارية أو حماية جوالك بعد اليوم! هذا الكفر المبتكر يجمع بين الحماية القوية والباور بانك المدمج، ليمنح جهازك الآيفون 13 Pro Max عمرًا أطول ومظهرًا أنيقًا.###FEATURES###بطارية مدمجة بسعة 6800mAh.
تصميم نحيف وأنيق وخفيف الوزن.
يشحن الجهاز لاسلكيًا بلمسة واحدة.
يحمي الجوال من الصدمات والخدوش.
شاشة LED لعرض مستوى الشحن.###DETAILS###السعة: 6800mAh
متوافق مع: iPhone 13 Pro Max (6.7 انش)
الخامة: ABS + TPU
نوع البطارية: ليثيوم بوليمر
الإدخال: 5V/1.5A
الإخراج: 5V/1.0A`,
    price: 169.99,
    images: getImages('product-30'),
    category: 'electronics'
  },
  {
    id: 'product-31',
    name: 'محطة شحن صغيرة',
    slug: 'mini-charging-station',
    description: `استمتع بالطاقة أينما كنت! محطة الشحن الصغيرة هذه تعمل بالطاقة الشمسية وتوفر لك مصدر طاقة محمول وموثوق لشحن أجهزتك وإضاءة مكانك، مثالية للرحلات والطوارئ.###FEATURES###تعمل بالطاقة الشمسية.
بطارية قابلة للشحن بسعة 4000mAh.
4 مستويات للإضاءة عبر 3 لمبات LED.
منفذ USB لشحن الجوال.
حماية من الجهد المنخفض والعالي.###DETAILS###الضمان: 14 يوم
البطارية: 6 فولت / 4 أمبير
مدة الشحن: 6-7 ساعات
مدة التشغيل: 4 ساعات تقريبًا
مخرج الشحن USB: 5 فولت / 1 أمبير`,
    price: 249.99,
    images: getImages('product-31'),
    category: 'home',
    warranty: '14 يوم'
  },
  {
    id: 'product-32',
    name: 'كاميرا على شكل لمبة',
    slug: 'light-bulb-camera',
    description: `أمان وذكاء في تصميم مبتكر! هذه الكاميرا الخفية على شكل لمبة تمنحك مراقبة شاملة لمنزلك أو مكتبك بدقة عالية ورؤية 360 درجة، كل ذلك مع سهولة تركيب لا مثيل لها.###FEATURES###تصوير بجودة عالية 1080p HD.
تركيب سهل مثل أي لمبة عادية.
تغطية 360 درجة ورؤية بانورامية.
تدعم الرؤية الليلية والتسجيل عند الحركة.
إمكانية التحكم والمشاهدة عبر تطبيق بالجوال.###DETAILS###الضمان: 14 يوم
الدقة: 1080p (فل اتش دي)
الاتصال: واي فاي
الرؤية الليلية: تصل إلى 10 أمتار
زاوية الحركة: 355° أفقي، 90° عمودي`,
    price: 149.99,
    images: getImages('product-32'),
    category: 'home',
    warranty: '14 يوم'
  },
  {
    id: 'product-33',
    name: 'جهاز الطاقة الشمسية + شريط الإضاءة',
    slug: 'solar-power-kit-with-light-strip',
    description: `طقم متكامل للطاقة والإضاءة في البر والرحلات! جهاز طاقة شمسية صغير الحجم مع بطارية قوية، بالإضافة إلى شريط إضاءة 10 أمتار كهدية. كل ما تحتاجه لإنارة وشحن أجهزتك بعيدًا عن الكهرباء.###FEATURES###جهاز طاقة شمسية مع بطارية 12 فولت/7 أمبير.
يشحن بالطاقة الشمسية خلال 10-12 ساعة.
منفذ USB لشحن الجوالات.
مخرج DC لتشغيل 4 لمبات LED مرفقة.
هدية: شريط إضاءة 10 متر للتخييم.###DETAILS###الضمان: 14 يوم
البطارية: 12 فولت / 7 أمبير ساعة
اللوح الشمسي: 18 فولت / 12 واط
مخرج USB: 5 فولت / 2 أمبير`,
    price: 349.99,
    images: getImages('product-33'),
    category: 'home',
    warranty: '14 يوم'
  },
  {
    id: 'product-34',
    name: 'خاتم ذكي معدني مع جهاز مراقبة الصحة',
    slug: 'smart-health-monitoring-ring',
    description: `تابع صحتك ولياقتك بأناقة لا مثيل لها! هذا الخاتم الذكي بتصميمه المعدني الأنيق يراقب نبضات قلبك، ونسبة الأكسجين في الدم، وجودة نومك، ويتتبع أكثر من 100 وضع رياضي.###FEATURES###يتتبع أكثر من 100 وضع رياضي.
مقاوم للماء بمعيار IP68 (مناسب للسباحة).
يراقب نبضات القلب، الأكسجين، النوم، والضغط.
شحن مغناطيسي سريع يدوم 3-5 أيام.
تصميم معدني أنيق وخفيف.###DETAILS###النظام: BT5.1
مقاومة الماء: IP68
البطارية: 18mAh (3-5 أيام)
الخامة: ستانلس ستيل
التطبيق: متوافق مع Android و iOS`,
    price: 299.99,
    images: getImages('product-34'),
    category: 'health'
  },
  {
    id: 'product-35',
    name: 'شريحة لتقوية شبكة الجوال',
    slug: 'mobile-signal-booster-sticker',
    description: `قل وداعًا لضعف الإشارة! هذه الشريحة الصغيرة والذكية تعمل على تحسين استقبال شبكة جوالك في الأماكن ضعيفة التغطية مثل المصاعد، الأنفاق، والمباني المغلقة. سهلة التركيب وفعالة.###FEATURES###تحسن استقبال الشبكة في الأماكن المغلقة.
تقلل التشويش أثناء المكالمات.
تعمل مع جميع الجوالات الذكية.
تصميم خفيف وصغير يسهل لصقه.
مناسبة للسيارات والمصاعد والمباني.###DETAILS###الضمان: 14 يوم
المادة: PCB عالية الجودة
الوزن: 10 جرام
متوافقة مع: جميع الأجهزة الذكية`,
    price: 49.99,
    images: getImages('product-35'),
    category: 'misc',
    warranty: '14 يوم'
  },
  {
    id: 'product-36',
    name: 'محول الشاشة العادية للأندرويد + مجموعة ألعاب',
    slug: 'android-tv-box-gaming-bundle',
    description: `حوّل تلفزيونك العادي إلى منصة ألعاب وترفيه ذكية! هذا الجهاز بنظام أندرويد يدعم دقة 8K ويأتي مع آلاف الألعاب الكلاسيكية وذراعي تحكم لاسلكية لتجربة لعب لا مثيل لها.###FEATURES###يدعم دقة عرض 8K Ultra HD.
يأتي مع ذراعي تحكم لاسلكية.
نظام أندرويد لتحميل التطبيقات ومشاهدة الأفلام.
آلاف الألعاب الكلاسيكية مدمجة.
توصيل سهل عبر HDMI.###DETAILS###النظام: أندرويد
الاتصال: 5G, HDMI, لاسلكي
يدعم: التلفزيونات، اللابتوبات، البروجكترات`,
    price: 349.99,
    images: getImages('product-36'),
    category: 'electronics'
  },
  {
    id: 'product-37',
    name: 'مترجم فوري شامل 5 في 1 بالذكاء الاصطناعي',
    slug: 'ai-5-in-1-translator',
    description: `اكسر حواجز اللغة مع هذا المترجم الفوري الشامل! يترجم النصوص والصور والأصوات من وإلى أكثر من 100 لغة بدقة وسرعة. جهاز لا غنى عنه للمسافرين والطلاب ورجال الأعمال.###FEATURES###يترجم النصوص والصور والأصوات.
يدعم أكثر من 134 لغة.
يعمل عبر الإنترنت ودون اتصال بالإنترنت.
شاشة لمس 5.0 بوصة عالية الدقة.
تصميم أنيق وسهل الحمل.###DETAILS###حجم الشاشة: 5.0 بوصة
الذاكرة: 8 جيجا بايت (يدعم بطاقة ذاكرة)
البطارية: ليثيوم
لغات الصوت: 134 لغة`,
    price: 499.99,
    images: getImages('product-37'),
    category: 'electronics'
  },
  {
    id: 'product-38',
    name: 'حقيبة السفر وطلعات البر',
    slug: 'travel-and-outdoor-backpack',
    description: `رفيقتك المثالية في كل مغامرة! حقيبة ظهر بسعة كبيرة ومقاومة للماء، مع تصميم مريح للظهر وجيوب متعددة ومنظمة، بما في ذلك جيب خاص للابتوب وآخر للأحذية.###FEATURES###سعة كبيرة تصل إلى 40 لتر.
مقاومة للماء والخدوش.
حافظة مبطنة للابتوب حتى 17 إنش.
جيب خاص منفصل للأحذية بفتحات تهوية.
تصميم مريح للظهر وحزام كتف مبطن.###DETAILS###الماركة: Hp hope
الخامة: بوليستر
السعة: 36 إلى 59 لتر
الأبعاد: 32 × 15 × 44 سم
الوزن: 1000 جرام`,
    price: 199.99,
    images: getImages('product-38'),
    category: 'misc'
  },
  {
    id: 'product-39',
    name: 'أصغر كاميرة أمان للعمل والمنزل',
    slug: 'mini-security-camera',
    description: `راقب ممتلكاتك بذكاء وسرية! هذه الكاميرا الصغيرة جدًا توفر لك تصويرًا عالي الدقة، رؤية ليلية، وتنبيهات حركة على جوالك. مثالية لمراقبة الأطفال، المكتب، أو المنزل دون أن يلاحظها أحد.###FEATURES###تصوير بجودة 1080p HD.
رؤية ليلية وتسجيل عند الحركة.
زاوية تصوير عريضة 180 درجة.
اتصال واي فاي للمشاهدة المباشرة من الجوال.
بطارية مدمجة وتدعم كرت ذاكرة.###DETAILS###الضمان: 14 يوم
الدقة: 1080P / 720P
زاوية التصوير: 180 درجة
الاتصال: واي فاي 2.4GHz
التخزين: تدعم كرت TF حتى 32GB`,
    price: 129.99,
    images: getImages('product-39'),
    category: 'electronics',
    warranty: '14 يوم'
  },
  {
    id: 'product-40',
    name: 'ميزان لتحليل جميع بيانات الجسم',
    slug: 'body-analysis-smart-scale',
    description: `أكثر من مجرد ميزان! هذا الجهاز الذكي يتصل بهاتفك عبر البلوتوث ليقدم لك تحليلًا شاملاً لجسمك، بما في ذلك نسبة الدهون، الماء، العضلات، والسعرات الحرارية.###FEATURES###يتصل بالجوال عبر البلوتوث.
شاشة LCD واضحة.
يقيس الوزن، الدهون، الماء، العضلات، وأكثر.
يدعم تطبيق ذكي لمتابعة التقدم.
يتحمل وزن حتى 180 كيلو.###DETAILS###الضمان: عامين
الاتصال: بلوتوث
سعة الوزن: حتى 180 كجم
البطارية: 2 × بطارية AAA
المواد: زجاج مقوّى`,
    price: 99.99,
    images: getImages('product-40'),
    category: 'health',
    warranty: 'عامين'
  },
  {
    id: 'product-41',
    name: 'محطة الراعي والمكشات 7 أمبير',
    slug: 'pastor-camping-station-7a',
    description: `طاقة لا تنقطع في قلب الطبيعة! هذه المحطة الشمسية هي الحل الأمثل للرحلات والكشتات، حيث توفر لك مصدر طاقة مستمر لشحن أجهزتك وإنارة مكانك عبر 4 لمبات مرفقة.###FEATURES###تعمل بالطاقة الشمسية مع بطارية 12 فولت/7 أمبير.
تشحن بالكامل خلال 10-12 ساعة من الشمس.
منفذ USB لشحن الجوالات والأجهزة الصغيرة.
مخرج DC لتشغيل 4 لمبات LED.
صغيرة الحجم وسهلة الحمل.###DETAILS###الضمان: 14 يوم
البطارية: 12 فولت / 7 أمبير ساعة
اللوح الشمسي: 18 فولت / 12 واط
مخرج USB: 5 فولت / 2 أمبير`,
    price: 329.99,
    images: getImages('product-41'),
    category: 'home',
    warranty: '14 يوم'
  },
  {
    id: 'product-42',
    name: 'باور بانك بسعة كبيرة 100 ألف مللي أمبير',
    slug: '100000mah-power-bank',
    description: `وداعًا لنفاذ البطارية! هذا الباور بانك العملاق بسعة 100,000 مللي أمبير قادر على شحن جميع أجهزتك عدة مرات. مع شحن سريع وكابلات مدمجة، هو مصدر الطاقة الذي لا غنى عنه في السفر والطوارئ.###FEATURES###سعة ضخمة 100,000mAh تكفي لأيام.
شحن سريع 22.5 واط.
كابلات USB و Type-C مدمجة.
4 مخارج لشحن أكثر من جهاز في نفس الوقت.
مؤشر LED لمعرفة مستوى البطارية.###DETAILS###السعة: 100,000mAh
الطاقة الخارجة: 22.5W
الكابلات: USB و Type-C مدمجة
الضمان: سنتين`,
    price: 399.99,
    images: getImages('product-42'),
    category: 'electronics',
    warranty: 'سنتين'
  },
  {
    id: 'product-43',
    name: 'budi power bank 20000',
    slug: 'budi-power-bank-20000',
    description: `طاقة إضافية في جيبك! باور بانك Budi بسعة 20,000 مللي أمبير وشحن سريع هو الحل الموثوق للحفاظ على أجهزتك مشحونة طوال اليوم. صغير، أنيق، وقوي.###FEATURES###سعة بطارية كبيرة 20,000mAh.
يدعم الشحن السريع.
منفذين USB لشحن جهازين في نفس الوقت.
تصميم صغير وسهل الحمل.
متوافق مع أغلب الجوالات والأجهزة.###DETAILS###الماركة: budi
الضمان: عامين
السعة: 20,000 مللي أمبير
عدد المنافذ: 2 منفذ USB
يدعم الشحن السريع`,
    price: 129.99,
    images: getImages('product-43'),
    category: 'electronics',
    warranty: 'عامين'
  },
  {
    id: 'product-44',
    name: 'محول الجوال إلى جهاز كمبيوتر',
    slug: 'mobile-to-pc-adapter-yesido',
    description: `أطلق العنان لقوة جوالك! هذا المحول من Yesido يضيف 6 منافذ عملية لجهازك (Type-C)، بما في ذلك USB 3.0 وقارئ بطاقات SD/Micro SD، لتنقل ملفاتك وتتصل بأجهزتك بسهولة وسرعة.###FEATURES###6 منافذ متعددة في جهاز واحد.
نقل بيانات سريع عبر USB 3.0.
قارئ بطاقات SD و Micro SD مدمج.
تصميم عملي ومحمول.
متوافق مع أجهزة Type-C.###DETAILS###العلامة التجارية: Yesido
نوع الموصل: Type-C HUB
عدد المنافذ: 6 منافذ
موديل الجهاز: HB10`,
    price: 119.99,
    images: getImages('product-44'),
    category: 'electronics'
  },
  {
    id: 'product-45',
    name: 'إضاءة دائرية بالطاقة الشمسية للحديقة',
    slug: 'solar-garden-circular-light',
    description: `أضف لمسة جمالية لحديقتك مع هذه الإضاءة الشمسية الأنيقة. تعمل تلقائيًا عند الغروب وتوفر إضاءة بلونين (أبيض دافئ أو ألوان متغيرة). مقاومة للماء وسهلة التركيب.###FEATURES###تعمل بالطاقة الشمسية ولا تحتاج كهرباء.
لونين للإضاءة: أبيض دافئ وألوان RGB.
تعمل تلقائيًا في الليل.
مقاومة للماء (IP44).
سهلة التركيب بالغرز في الأرض.###DETAILS###الضمان: 14 يوم
نوع الإنارة: LED
لومن: 60 لومن
البطارية: NiMH 300mAh
العمر الافتراضي: 30,000 ساعة`,
    price: 59.99,
    images: getImages('product-45'),
    category: 'home',
    warranty: '14 يوم'
  },
  {
    id: 'product-46',
    name: 'إضاءة مكتب',
    slug: 'office-light',
    description: `إضاءة ذكية لمكتبك! تعمل بحساس حركة وتوفر 8 ألوان مختلفة لتناسب مزاجك. لا تعمل إلا في الظلام لتوفير الطاقة. مثالية للقراءة والعمل دون إجهاد للعين.###FEATURES###إضاءة LED بـ 8 ألوان مختلفة.
تعمل بحساس حركة (على بعد 2-3 متر).
تعمل بـ 3 بطاريات AAA (غير مشمولة).
لا تعمل في وجود ضوء لتوفير الطاقة.
سهلة التركيب على طرف المكتب.###DETAILS###نوع الإضاءة: LED
مصدر الطاقة: 3 بطاريات AAA
الخامة: بلاستيك ABS`,
    price: 39.99,
    images: getImages('product-46'),
    category: 'home'
  },
  {
    id: 'product-47',
    name: 'طاولة متعددة الاستخدامات اصفر خشبي',
    slug: 'multi-use-folding-table-yellow-wood',
    description: `رفيقتك المثالية في الرحلات والمنزل! طاولة خفيفة وقابلة للطي، تتحمل وزن حتى 30 كجم وتوفر مساحة تخزين داخلية. استخدمها للابتوب، للأكل، أو للدراسة.###FEATURES###خفيفة وقابلة للطي.
تتحمل وزن حتى 30 كجم.
سطح واسع يناسب لابتوبات حتى 17 إنش.
مساحة تخزين داخلية.
سهلة التنظيف والحمل.###DETAILS###الضمان: 14 يوم
الأبعاد: 60×40×30 سم
الخامة: إطار ألمنيوم مع سطح MDF
الوزن: 1.3 كجم`,
    price: 89.99,
    images: getImages('product-47'),
    category: 'home',
    warranty: '14 يوم'
  },
  {
    id: 'product-48',
    name: 'حقيبة مقاومة للماء حديثة',
    slug: 'modern-waterproof-bag',
    description: `أناقة وأمان في حقيبة واحدة. هذه الحقيبة العصرية المقاومة للماء تحمي أغراضك وتوفر لك منفذ USB لشحن جوالك أثناء التنقل. مثالية للاستخدام اليومي والدوام والسفر.###FEATURES###تصميم أنيق ومقاوم للماء.
سحاب مخفي ونظام ضد السرقة.
منفذ USB مدمج لشحن الهاتف.
خامة نايلون قوية ومتينة.
خفيفة على الكتف ومناسبة للجنسين.###DETAILS###الماركة: yunfan
النوع: حقيبة كتف
الأبعاد: 17 × 32 × 7 سم
الخامة: نايلون مقاوم للماء + PU`,
    price: 79.99,
    images: getImages('product-48'),
    category: 'misc'
  },
  {
    id: 'product-49',
    name: 'ساعة رياضية ذكية',
    slug: 'smart-sports-watch',
    description: `شريكك في رحلتك الصحية! ساعة رياضية ذكية تقيس نبضات قلبك، خطواتك، السعرات الحرارية، وجودة نومك. توصل لك إشعارات جوالك وتبقى مشحونة لأكثر من 15 يومًا في وضع الاستعداد.###FEATURES###تقيس نبضات القلب والخطوات والسعرات.
تراقب جودة النوم.
تستقبل إشعارات المكالمات والرسائل.
بطارية تدوم لأكثر من 15 يومًا (استعداد).
مقاومة لرذاذ الماء.###DETAILS###الموديل: Shivay M3 Smart Band
الضمان: 14 يوم
الاتصال: بلوتوث 4.0
التوافق: أندرويد 4.3+ / iOS 7.1+`,
    price: 89.99,
    images: getImages('product-49'),
    category: 'health',
    warranty: '14 يوم'
  },
  {
    id: 'product-50',
    name: 'بنك طاقة لاسلكي 10000 مللي أمبير',
    slug: 'yosonda-10000mah-wireless-power-bank',
    description: `اشحن كل أجهزتك بلمسة واحدة! بنك الطاقة من YOSONDA يدعم الشحن اللاسلكي السريع مع شفاطات لتثبيت هاتفك، بالإضافة إلى كابلات مدمجة لجميع أنواع الأجهزة.###FEATURES###يدعم الشحن اللاسلكي بقدرة 10 واط.
شفاط لتثبيت الهاتف أثناء الشحن اللاسلكي.
كابلات مدمجة (Type-C, Micro USB, Lightning, USB).
سعة 10000 مللي أمبير.
خفيف ومحمول ومناسب للرحلات.###DETAILS###العلامة التجارية: YOSONDA
الموديل: YXD-A40
البطارية: 10000 مللي أمبير
المخرج اللاسلكي: 10 واط`,
    price: 119.99,
    images: getImages('product-50'),
    category: 'electronics'
  },
  {
    id: 'product-51',
    name: 'حامل جوال متعدد الاستخدمات',
    slug: 'multi-use-phone-holder',
    description: `حرر يديك واستمتع بالمشاهدة! هذا الحامل المرن يمكنك تعليقه على رقبتك أو وضعه على الطاولة لمشاهدة مريحة. يدور 360 درجة ويناسب معظم الجوالات والأجهزة اللوحية.###FEATURES###مرن ويتشكل بأي طريقة تريدها.
رأس يدور 360 درجة.
يناسب جوالات وتابلت من 4 إلى 9.5 إنش.
يمكن تعليقه على الرقبة أو تثبيته على سطح.
يخفف آلام الرقبة واليدين.###DETAILS###الضمان: 14 يوم
الخامة: ألمنيوم + بلاستيك
الوزن: 218 جرام
متوافق مع: معظم الجوالات والتابلت`,
    price: 59.99,
    images: getImages('product-51'),
    category: 'misc',
    warranty: '14 يوم'
  },
  {
    id: 'product-52',
    name: 'أيربودز حديثة',
    slug: 'modern-airpods',
    description: `تجربة صوتية غامرة واتصال فائق السرعة! هذه الأيربودز الحديثة بتقنية بلوتوث 5.4 تمنحك صوتًا محيطيًا نقيًا، ومكالمات واضحة بفضل مايك عازل للضوضاء، وبطارية تدوم لساعات طويلة.###FEATURES###بطارية تدوم حتى 72 ساعة مع العلبة.
تقنية بلوتوث 5.4 لاتصال سريع ومستقر.
صوت محيطي 360 درجة.
مايك ENC يعزل الضوضاء للمكالمات.
مقاومة للتعرق ومريحة للأذن.###DETAILS###الضمان: 14 يوم
الإصدار: بلوتوث 5.4
سعة البطارية: 30mAh لكل سماعة
معيار مقاومة الماء: IPX-5`,
    price: 139.99,
    images: getImages('product-52'),
    category: 'electronics',
    warranty: '14 يوم'
  },
  {
    id: 'product-53',
    name: 'أبجورة فندقية ذكية',
    slug: 'smart-hotel-style-lamp',
    description: `أضف لمسة من الذكاء والأناقة لمكتبك! هذه الأبجورة الفندقية توفر إضاءة مريحة للعين، شاشة LED لعرض الوقت، شاحن لاسلكي سريع لهاتفك، وحامل مدمج، كل ذلك في تصميم واحد أنيق.###FEATURES###إضاءة LED بثلاثة ألوان و سطوع متغير.
شاشة LED تعرض الساعة والمنبه.
شحن لاسلكي سريع 15 واط.
حامل جوال مدمج.
ذراع مرن يتحرك 180 درجة.###DETAILS###الضمان: 14 يوم
نوع التحكم: لمس
الشحن اللاسلكي: 15 واط
مادة التصنيع: ألومنيوم وبلاستيك ABS`,
    price: 189.99,
    images: getImages('product-53'),
    category: 'home',
    warranty: '14 يوم'
  },
  {
    id: 'product-54',
    name: 'طاولة متعددة الاستخدامات لون أبيض',
    slug: 'multi-use-folding-table-white',
    description: `رفيقتك المثالية في الرحلات والمنزل! طاولة خفيفة وقابلة للطي، تتحمل وزن حتى 30 كجم وتوفر مساحة تخزين داخلية. استخدمها للابتوب، للأكل، أو للدراسة.###FEATURES###خفيفة ومتينة وقابلة للطي.
تتحمل وزن يصل إلى 30 كجم.
سطح واسع يناسب لابتوبات حتى 17 إنش.
مساحة تخزين داخلية مخفية.
سهلة التنظيف والحمل.###DETAILS###الأبعاد: 60×40×30 سم
الخامة: إطار ألمنيوم مع سطح MDF
الوزن: 1.3 كجم
التحمل الأقصى: 30 كجم`,
    price: 89.99,
    images: getImages('product-54'),
    category: 'home'
  },
  {
    id: 'product-55',
    name: 'جراب متين ضد الماء',
    slug: 'durable-waterproof-pouch',
    description: `لا تخف على جوالك في الماء بعد الآن! هذا الجراب المتين يطفو على سطح الماء ويحميه حتى عمق 30 مترًا، مع إمكانية استخدام الشاشة والتصوير بوضوح تام.###FEATURES###يطفو فوق الماء ويمنع الجوال من الغرق.
مقاوم للماء حتى عمق 30 مترًا (IP68).
يمكن استخدام الشاشة والتصوير من خلاله.
يناسب أغلب أنواع الجوالات.
يأتي مع حبل تعليق.###DETAILS###الماركة: MOOGMAX
الضمان: 14 يوم
معيار الحماية: IP68
الخامة: PVC ناعم وقوي`,
    price: 49.99,
    images: getImages('product-55'),
    category: 'misc',
    warranty: '14 يوم'
  }
];

// Helper function to create product lists for different categories
const createProductList = (products: Product[], category: string): Product[] => {
    return products.filter(p => p.category === category);
};

// Create lists for each category
export const electronicsProducts: Product[] = createProductList(allProducts, categories.electronics.slug);
export const homeProducts: Product[] = createProductList(allProducts, categories.home.slug);
export const healthProducts: Product[] = createProductList(allProducts, categories.health.slug);
export const miscProducts: Product[] = createProductList(allProducts, categories.misc.slug);


// Combine all product arrays here
export const products: Product[] = [
    ...allProducts
];
